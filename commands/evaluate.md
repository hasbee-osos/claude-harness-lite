...
2. Dispatch the `engineering-harness:evaluator` subagent with the ticket, iteration number, `state.json`, and paths to all artifacts (analysis, design, implementation report). The evaluator inspects every changed repo, its diff, the evidence, and cross-repo consistency independently.
