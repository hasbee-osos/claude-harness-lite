...
6. **Analyzer.** Dispatch `engineering-harness:analyzer` with the ticket, Jira context, repo list and `source_branch`. Write the artifact to `sis-brain/tickets/<ticket-id>/analysis.md`. Lock a decision for the root cause. Update `state.json` (`analysis`, `status`, `artifacts`, `next_action`). If `NEEDS_INPUT`, present the open questions and stop.
7. **Designer.** Dispatch `engineering-harness:designer` with the ticket, the analysis and the locked decisions. Write to `sis-brain/tickets/<ticket-id>/design.md`. Append the designer's decision records to `decisions.md`. Update `state.json`. If `NEEDS_INPUT`, stop and escalate.
