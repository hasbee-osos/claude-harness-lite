...
3. **Inspect repository state.** Run `git status` and `git branch --show-current`.
   ...
   - If already on a branch for this ticket (or `state.json` records one), reuse it.
   - Otherwise, follow the `git-workflow` skill's flow selection and branch naming: propose a flow and branch name, **wait for the human to confirm both**, then create the branch from that flow's source branch. If that could destroy work, stop and ask.
4. **Initialize runtime state.** Create `.runtime/<ticket-id>/` with `state.json` (fields: `ticket`, `status`, `iteration`, `max_iterations: 3`, `flow`, `source_branch`, `branch`, `pr_targets`, `analysis`, `design`, `implementation`, `evaluation`). If a previous run's `.runtime/<ticket-id>/` exists, resume from its recorded status instead of starting over.
...
10. **PR.** Follow the `/pr` command logic: verify `evaluation: PASS` in `state.json`, then raise the PRs for the **first stage** in `pr_targets` only (later stages are raised by re-running `/pr` once the human confirms the previous stage is done, per `git-workflow`) via `gh` if authenticated, otherwise via the `/pr` PR link fallback (push branch, compare link + description for the human to open the PR). **Never merge.**
