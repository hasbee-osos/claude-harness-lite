...
4. **Work type, line and branch name.** Propose the work type from the Jira issue type (`harness-core` → Work types), and, from the issue's **Customer Name** field (`git-workflow` → The routing decision), the line, branch name (`bugfix` or `feature`), source branch and PR target. **Wait for the human to confirm**, then record **Work type, line and branch confirmed**. If Customer Name is empty or unrecognised, do not guess: write the input packet (`sme-packet.md` for a feature, `qa-packet.md` for a bug) and stop. Hotfix tickets: escalate.
...
9a. **Conflict check** (`git-workflow` → Merge conflicts). For each changed repo, `git -C <repo> fetch origin`, then `conflicts.js check` against `origin/<pr_target>` — with the resolve branch as head if `state.json` records one for that repo, otherwise the ticket branch. Record **Conflicts checked**.
    - Clean, and any recorded resolve branch already contains the ticket branch's head → step 10.
    - Otherwise dispatch `engineering-harness:implementor` in **resolve mode** with the `check` output, the plan, `decisions.md` and the resolve branch to create (`conflicts.js name`) or reuse. If it returns semantic hunks, present them to the developer as one bundled question — what each side intended and the proposed resolution — record each confirmed resolution as a decision citing both Jira keys, and dispatch resolve mode again with them. Record **Conflicts resolved**, then step 10. If the equivalence check still fails, the resolution is not done: dispatch resolve mode again with the failures, or stop and ask.
10. **Evaluator.** Dispatch `engineering-harness:evaluator` with the ticket, all artifacts, `decisions.md`, `state.json` and the current iteration — and, for each repo with a resolve branch, the full `conflicts.js compare … --record` command to re-run. Record **Evaluation returned**.
...
12. **Raise the PR against `pr_target`**, for every changed repo.
    - **Gate:** either the latest verdict is `PASS` and every changed repo's `HEAD` equals its `evaluated_head`, or the evaluation rounds are used up, the final fix round reported every blocking finding closed, and every changed repo's `HEAD` equals its `final_head`. A recorded resolve branch must likewise be at `resolve.evaluated_head`, or at a `resolve.head` recorded by a post-evaluation resolution below. Otherwise handle it as the `PR_STAGE_1` resume row says. Never skip this gate.
    - **Conflicts, again.** For every changed repo, fetch and `check` the PR head (resolve branch or ticket branch) against `origin/<pr_target>` once more, since the target moves while the ticket is evaluated. If it now conflicts, or a recorded resolve branch lacks the ticket branch's head (a final fix round landed after it), dispatch resolve mode **after evaluation**: mechanical hunks only; any semantic hunk goes to the developer as in step 9a. No evaluation round follows. Record **Conflicts resolved** and add a **Conflicts resolved after evaluation — reviewer to check** section to that repo's PR: each hunk, its rationale, and the `compare` table.
...
4. Record **New iteration** with the feedback as its reason and run step 9 with that feedback (and any re-plan delta) as scope, then step 9a, then step 10: one implement pass and one evaluation, whatever the track. Evaluation is never skipped; a `FAIL` follows step 11 with the rounds left. Step 9a matters here: the target has usually moved since the PR was opened, and a resolve branch must take in the fix.
5. Push the ticket branch (never force), and the resolve branch for any repo whose PR comes from one — that is the branch the open PR shows. An open PR updates by itself. Raise a PR only for a repo that has none. Record **PRs prepared**, then **Run stops**.
