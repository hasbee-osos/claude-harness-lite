...
3. **Initialize or resume.** Discover the workspace repos (`workspace`).
   - `git -C .brain pull --rebase` first, so the record includes what colleagues have pushed. If `.brain` is missing or is not a git repo, stop and ask the human to clone the brain repo.
   ...
   - Otherwise create `.brain/tickets/<ticket-id>/`, seed any missing repo-root files per `brain` (`.harness-brain`, `README.md` from `templates/brain-readme.md`, `.gitignore`, `.gitattributes`), write `state.json`, append `ticket_started`, then commit and push.
...
15. **Close the run.** Append a line to `.brain/index.jsonl` (ticket, status, verdict, iterations, repos, branch, PR count), set `next_action` to what the human does next, write `{}` to `.brain/current.json`, then commit and push the brain one last time.
