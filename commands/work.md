...
2. **Load Jira context.** Read the issue through the read-only Jira MCP (`view: "evidence"`, so custom fields such as *Dev Lead Estimation* are included): summary, description, comments (`executeRead` → `listJiraIssueComments`), linked issues, and its sub-tasks' estimate and logged time for the `jira` block (`brain` → `state.json`). **If Jira is unavailable, stop** and explain that the context could not be retrieved. Never fabricate ticket content. If no Jira MCP is configured, ask the user to paste the ticket and record that it was provided manually.
