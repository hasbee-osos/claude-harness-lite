...
Input: optional `$ARGUMENTS` (Jira ticket ID). Read the `brain` skill first. **This command is read-only**: it never changes ticket state, never dispatches an agent, never touches a product repo. It may run `git -C .brain pull --rebase` to show the team's current record, and nothing else.

## With no argument — list recent work

1. `git -C .brain pull --rebase`, then read the last ~30 lines of `.brain/index.jsonl`. If it does not exist, list `.brain/tickets/*/state.json` instead and say the index is missing.
...
3. Show `.brain/current.json` if it names a ticket, as "in progress now".
4. If `.brain/` does not exist, say so plainly — the harness has not run in this workspace yet — and stop.

## With a ticket ID — show that ticket

`git -C .brain pull --rebase`, then read `.brain/tickets/<ticket-id>/` and present, in this order:
