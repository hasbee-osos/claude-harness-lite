...
Input: optional `$ARGUMENTS` (Jira ticket ID). Read the `brain` skill first. **This command is read-only**: it never changes state, never dispatches an agent, never touches a repo.

## With no argument — list recent work

1. Read the last ~30 lines of `.brain/index.jsonl`. If it does not exist, list `.brain/tickets/*/state.json` instead and say the index is missing.
...
Read `.brain/tickets/<ticket-id>/` and present, in this order:
