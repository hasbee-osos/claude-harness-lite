...
2. Discover the workspace repos (`workspace` skill). If `sis-brain/tickets/<ticket-id>/state.json` has no `work_type`/`flow`/`source_branch`, propose them (work type per `work-types`) with the branch name per `git-workflow` and **wait for the human to confirm**; record them in `state.json` and in `human_confirmations`, and lock a decision record for the flow choice (after the ignore check in `brain`). Run `git -C <repo> fetch origin` for every repo. Do not create branches or change any checkout.
...
4. Write the returned artifact to `sis-brain/tickets/<ticket-id>/analysis.md`, lock a decision record for the root cause (bug) or the confirmed scope and acceptance criteria (feature), and update `state.json` (`analysis: READY` or the NEEDS_INPUT status, plus `artifacts`, `updated_at` and `next_action`). Journal `stage_start`/`stage_end` around the dispatch.
