...
- Independently establish: Does the ticket appear solved? For a **bug**: is the fix proven by a regression test that exercises the root cause? For a **feature**: is **every acceptance criterion** met, with executed evidence for each, and was nothing built beyond the confirmed scope (`harness-core` → Work types)? Does the implementation match the plan (or justified deviations)? Does every changed or added method, class or component have updated or new unit tests for the changed behaviour (`engineering-standards` → Testing)? A touch point without them is a blocking finding unless the report gives an exception the standard allows. Is the root cause or scope in the plan actually right — a regression test that passes without the fix, or behaviour the ticket describes that the change does not address, is a blocking finding. Are important regressions addressed? Were the relevant tests **actually executed**? Is the evidence sufficient? Are there material risks? Is anything blocking human review?
...
- Read the **lessons the plan cites** (`sis-brain/lessons/`). A `confirmed` lesson contradicted without a stated reason is treated exactly like a contradicted locked decision: a blocking finding, because the team already paid for that mistake once.
...
- Check the **locked decisions** in the ticket's `decisions.md`. Every LOCKED record must either be followed, or superseded by a later record that gives a reason. **An implementation that contradicts a LOCKED decision without superseding it is a blocking finding**, even if the code works — an undocumented reversal is exactly what makes a reopened ticket unexplainable. Also flag a decision record with no evidence, and a convention deviation implemented without a record. **On a round that followed a QA defect**, every decision the defect implicates must have been confirmed by the re-plan or superseded; one silently kept is a blocking finding.
- **Where a repo's PR will come from a resolve branch** (`state.json` records one), evaluate the resolution as part of the change: read the implementation report's **Conflict resolution** section and the record it names, re-run the `conflicts.js compare … --record` command the orchestrator passes you, and inspect each recorded hunk in `git -C <repo> diff origin/<pr_target>...<resolve-branch>`. A failing equivalence check, a hunk classed mechanical that is really semantic, or a semantic resolution without a developer-confirmed decision is a blocking finding.
