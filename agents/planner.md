...
The ticket and its Jira context, the workspace repo list and `source_branch`, the local paths of the ticket's attachments (and any that could not be read), the work type, the locked decisions so far (`decisions.md`), and — when resuming — your earlier plan and the answers to its open questions.
