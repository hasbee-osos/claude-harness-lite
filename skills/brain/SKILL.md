...
- **Generated indexes.** `codebase/build.js` reads every repo in `codebase/repos.json` at its `ref` through git, never the checkout, and writes `codebase/generated/`. That folder is gitignored and stamped with each repo's commit in `generated/stamp.json`. It holds screens (menu label per customer line → route → component), frontend services → backend controllers, routes, components, endpoints with their grants, tables, menus and outbound clients. It extracts names and paths only, never config values.
- **Rebuild** at `/work` step 5, after the fetch: `node sis-brain/codebase/build.js`. This is best-effort. If `build.js` is missing, skip it silently. If it fails, say so in one line, carry on, and tell the Planner the map may be stale.
