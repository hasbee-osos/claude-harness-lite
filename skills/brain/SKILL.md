...
| **Conflicts checked** (per repo, `git-workflow` → Merge conflicts) | — | `conflicts_checked` | — | with the next milestone |
| **Conflicts resolved** (per repo) | `conflict-resolution-<repo>-<target>.json`; `decisions.md` for each developer-confirmed semantic resolution | `stage_start`, `stage_end` (stage `resolve`), `conflicts_resolved`, `decision_locked` if any | `repos.<repo>.resolve` (`target`, `branch`, `head`, `record`, `after_evaluation`) | `<ID>: conflicts resolved - <repo>` |
| **Evaluation returned** | `evaluation-<n>.md`, `decisions.md` (verdict) | `stage_start`, `stage_end`, `evaluation`, `decision_locked` | `evaluation`, `artifacts`, `status`, `repos.<repo>.evaluated_head` (the commit evaluated in each repo) and `repos.<repo>.resolve.evaluated_head` where a resolve branch exists | `<ID>: evaluation <n> - <VERDICT>, <k> blocking` |
...
```json
{
  ...
  "jira": {
    ...
    "labels": ["SIS-GCET-QA", "SIS-GUTECH-QA"],
    "subtasks": [{ "key": "GSIS-12346", "type": "Dev Sub-task", "estimate": "2h", "logged": "1h 30m" }],
  ...
  "repos": {
    ...
    "sis-product-sis-frontend":      { "role": "change", "branch_created": true, "evaluated_head": "a41d07b", "final_head": null,
                                       "resolve": { "target": "base-sandbox-qa", "branch": "base/bugfix/GSIS-12345-short-desc-base-sandbox-qa-conflict-resolved", "head": "c07e91a", "evaluated_head": "c07e91a", "record": "conflict-resolution-sis-product-sis-frontend-base-sandbox-qa.json", "after_evaluation": false } }
  },
...
| `conflicts_checked` | a dry-run merge of a changed repo against its PR target (`git-workflow` → Merge conflicts) | `repo`, `target`, `head` (the branch checked), `result` (`clean` or `conflicts`), `files` (count) |
| `conflicts_resolved` | a resolve branch is created or brought up to date | `repo`, `target`, `branch`, `mechanical`, `semantic` (hunk counts), `equivalence` (`pass` or `fail`), `after_evaluation` |
