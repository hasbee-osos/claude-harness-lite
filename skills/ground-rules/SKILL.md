...
- **The brain is the durable record.** Every run writes its state, journal, decisions and artifacts to its ticket folder in `<workspace>/sis-brain/` (see `brain`, the single specification of what is recorded), so any later session can resume the ticket and any reader can see why the change was built this way. Iteration artifacts are numbered, never overwritten. Never write chain-of-thought, secrets or bulk file dumps there, and never commit the brain into a product repo.
- **Decisions are locked and citable.** Flow and work type, repos, root cause (bug) or scope and acceptance criteria (feature), fix or design approach, test strategy, convention deviations and the evaluator verdict each become a numbered decision record with its justification and evidence. Later stages cite the ID; changing course requires a new record that supersedes the old one. Contradicting a locked decision without superseding it is a blocking evaluator finding.
...
- **Follow project-specific conventions.** The team's written guidelines (`engineering-standards` and the references it links) outrank generic best practice; an unjustified breach is a blocking evaluator finding. Do not invent architecture. Make the smallest change that completely and safely delivers the ticket — a narrow fix for a bug, exactly the acceptance criteria for a feature. Document unrelated problems as findings; do not fix them.
- **Read the relevant skills before acting** (`workspace`, `brain`, `work-types`, `jira-attachments`, `input-packets`, `repository-analysis`, `architecture`, `testing`, `characterization-testing`, `git-workflow`, `engineering-standards`, and the project skills: `springboot`, `angular`, `postgresql`).
