...
- **Unit tests for every touch point.** Every production method, class or component a change modifies or adds gets unit tests for the changed behaviour: update the existing tests that cover it, and create the test class (`<Class>Test.java`) or spec (`<name>.spec.ts`) when none exists. The only exceptions are changes with no behaviour (renames, imports, formatting, configuration values) and code that genuinely cannot be unit-tested, and each is named in the implementation report with its reason. If the repo's test runner cannot run in the environment, still write the tests, and report them as written but not executed.
